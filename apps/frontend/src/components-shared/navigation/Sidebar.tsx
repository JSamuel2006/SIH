...
export default function Sidebar({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) {
  ...
  return (
    <aside
      className={`fixed lg:static inset-y-0 left-0 z-30 w-64 bg-slate-900 border-r border-slate-800 p-4 flex flex-col justify-between transition-transform duration-200 ${
        isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}
    >
      <div className="space-y-6">
        <div className="px-2 pt-2">
          <div className="text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-2">Portal Navigation</div>
      ...
      <div className="p-3 bg-slate-950/60 border border-slate-800 rounded-xl text-xs">
        <div className="font-bold text-slate-200">ArogyaVerse v1.0</div>
        <div className="text-[11px] text-slate-400 mt-0.5">ICMR & MoHFW Aligned</div>
      </div>
    </aside>
  );
}
